...
class DTFDMIL(nn.Module):
    ...
    def _forward_one(
        ...
        if self.training and self.pseudo_bags > 1 and n_instances >= self.pseudo_bags:
            ...
            pseudo_embeddings = [
                self._attention_pool(chunk, self.local_attention)
                for chunk in chunks
                if chunk.numel() > 0
            ]
    ...
    def __init__(
        ...
        if model_name == "dtfd_mil":
            ...
            self.model = DTFDTopKMIL(
                input_dim, embed_dim=embed_dim, attn_dim=attn_dim,
                pseudo_bags=dtfd_pseudo_bags, dropout=dropout, n_classes=n_classes,
                top_k=dtfd_top_k, eval_group_seed=dtfd_eval_group_seed,
            )
        else:
