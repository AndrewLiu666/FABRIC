...
class DTFDTopKMIL(DTFDMIL):
    ...
    def __init__(self, input_dim: int, embed_dim: int = 512, attn_dim: int = 256,
                 pseudo_bags: int = 8, dropout: float = 0.25, n_classes: int = 2,
                 top_k: int = 1, eval_group_seed: int = 42):
        if n_classes != 2:
...
def forward_topk_bags(model, bags, labels, criterion, device):
    ...
    for bag, label in zip(bags, labels):
        ...
        pseudo_losses.append(F.cross_entropy(
            pseudo, label.expand(len(pseudo)), weight=criterion.weight, reduction="none",
        ).mean())
