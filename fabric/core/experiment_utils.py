...
from .training import (
    compute_metrics, metric_rows_from_predictions, summarize_outputs,
)
...
MODEL_SPECS = {
    f"{key}_dtfd": ModelSpec(
        key=f"{key}_dtfd", display_name=f"{encoder} + DTFD-MIL FedAvg epoch5",
        encoder=encoder, mil_label="DTFD-MIL", model_name="dtfd_mil",
        source_run_dir=Path(), out_dir=Path(),
    )
...
def normalized_result_row(spec: ModelSpec, row: dict, fold: int, pred_df: pd.DataFrame, threshold: float, out_dir: Path) -> dict:
    row = dict(row)
    test_loss = float(row.get("test_loss", row.get("loss", float("nan"))))
    metrics = compute_metrics(pred_df["recurrence_label"], pred_df["prob_recurrence"], threshold=float(threshold), loss=test_loss)
    row.update(
        {
            "fold": int(fold),
            "model": spec.key,
...
def write_mean_std(fold_results: pd.DataFrame, spec: ModelSpec, out_dir: Path) -> pd.DataFrame:
    row = {
        ...
        "folds": ",".join(str(int(f)) for f in sorted(pd.to_numeric(fold_results["fold"]).unique())) if not fold_results.empty else "",
    }
    for metric in METRICS:
        values = pd.to_numeric(fold_results[metric], errors="coerce") if metric in fold_results else pd.Series(dtype=float)
