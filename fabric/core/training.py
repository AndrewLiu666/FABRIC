...
def collate_patient_bags(batch: Sequence[tuple[torch.Tensor, torch.Tensor, str]]) -> tuple[list[torch.Tensor], torch.Tensor, list[str]]:
    bags, labels, patient_ids = zip(*batch)
...
def compute_metrics(labels: Sequence[int], probs: Sequence[float], threshold: float, loss: float = float("nan")) -> dict:
...
def train_one_epoch(
    ...
    log = {"loss": total_loss / max(total_count, 1), "n": int(total_count)}
    if use_topk:
        log.update(patient_loss=patient_loss_total / max(total_count, 1),
                   pseudo_loss=pseudo_loss_total / max(total_count, 1))
    return log
...
def evaluate_model(
    ...
    metrics = compute_metrics(labels_all, probs_all, threshold=float(args.threshold), loss=total_loss / max(total_count, 1))
...
def metric_rows_from_predictions(pred_df: pd.DataFrame, fold: int | str, test_set: str, group_cols: Sequence[str] = ("institution",)) -> list[dict]:
    rows = []
...
def summarize_outputs(all_predictions: list[pd.DataFrame], fold_rows: list[dict], out_dir: Path, test_set: str) -> None:
    ...
    pooled_rows = metric_rows_from_predictions(pred_df, fold="pooled", test_set=test_set, group_cols=("institution", "site"))
