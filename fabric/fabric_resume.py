...
from fabric_common import (
    SITES, cpu_state, inside, read_json, sha256_file, state_digest, training_args, write_json,
)
...
def validate_resume_run(root: Path, run_root: Path, setups: list, folds: list,
                        manifest_root: Path) -> None:
    if not run_root.is_dir():
        raise FileNotFoundError(f"Resume run does not exist: {run_root}")
    # The wrappers below implement recovery. Model, data, seed, optimizer and
    # averaging source must still be identical to the original run.
    recovery_files = {"fabric/run_fabric.py", "fabric/fabric_client.py",
                      "fabric/fabric_aggregator.py", "fabric/fabric_resume.py", "fabric/check_static.py"}
    for relative, digest in read_json(run_root / "code_sha256.json").items():
        if relative not in recovery_files and sha256_file(inside(root, relative)) != digest:
    ...
    for setup in setups:
        ...
        expected.update(variant=setup.get("variant", "pooling"), model_options=setup.get("model_options", {}),
...
def run_lock(run_root: Path):
    ...
    for entry in Path("/proc").iterdir():
        ...
        same_name = (f"--run-name={run_root.name}" in argv or any(
            a == "--run-name" and b == run_root.name for a, b in zip(argv, argv[1:])))
...
def inspect_fold(fold_dir: Path, setup: dict) -> dict:
    ...
    if (Path(runtime["fold_dir"]).resolve() != fold_dir.resolve()
    ...
    paths = sorted((fold_dir / "server_audit").glob("round_*.json"),
    ...
    for number, audit_path in enumerate(paths, 1):
        ...
        if (audit_path.name != f"round_{number}.json" or audit["round"] != number
                ...
                or audit["input_state_sha256"] != digests[-1] or len(audit["local_logs"]) != len(SITES)):
    ...
    checkpoint = next((p for p in candidates if p.is_file()
                       and _load_matching(p, digests[completed], runtime["state_spec"]) is not None), None)
    if checkpoint is None:
        raise RuntimeError(f"No intact checkpoint matches audited round {completed} in {fold_dir}")
    required = ("nvflare_run.json", "global_model_round_final.pt", "global_test_predictions.tsv",
                "fold_result.tsv", "round_logs.tsv")
    action = "resume"
    if completed == setup["settings"]["rounds"]:
        ...
        if action == "complete":
            ...
            if _load_matching(fold_dir / "global_model_round_final.pt", digests[completed],
                              runtime["state_spec"]) is None:
                raise ValueError("Completed fold's final checkpoint is corrupt or changed")
    return {"action": action, "completed_rounds": completed, "checkpoint": str(checkpoint),
...
def prepare_resume(fold_dir: Path, plan: dict) -> tuple[Path, Path]:
    ...
    runtime.update(resume_offset=completed, resume_checkpoint=str(checkpoint),
                   resume_state_sha256=plan["state_sha256"])
