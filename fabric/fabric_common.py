...
LOCKED_SETTINGS = {
    "n_folds": 5, "rounds": 5, "local_epochs": 5, "batch_size": 32,
    "num_workers": 1, "max_instances": 4000, "lr": 1e-5,
    "weight_decay": 1e-4, "threshold": 0.5, "seed": 42,
    "device": "cuda", "gpu": 0, "amp": True, "embed_dim": 512,
    "attn_dim": 256, "dropout": 0.25, "dtfd_pseudo_bags": 8,
}
...
def load_data_config(repo_root: Path, path: Path) -> dict:
    ...
    return {
        ...
        "feature_path_prefix_map": {str(old): str(external_path(repo_root, new))
...
def remap_feature_paths(frame, prefix_map: dict[str, str]):
    ...
    frame = frame.copy()
    if prefix_map:
        frame["feature_paths"] = frame["feature_paths"].map(
            lambda value: remap_feature_value(value, prefix_map)
        )
    return frame
...
def load_setup(root: Path, experiment: str, variant: str = "pooling", top_k: int | None = None) -> dict:
    ...
    if top_k is not None and (variant != "topk" or isinstance(top_k, bool)
    ...
    expected = {
        ...
        "sites": list(SITES), "folds": list(range(5)), "nvflare_version": "2.7.2",
    }
    ...
    if variant == "topk":
        setup.update(variant="topk", model_name="dtfd_topk", model_key=f"{experiment}_dtfd_topk",
                     model_options={"top_k": top_k if top_k is not None else 1,
...
def _manifest_paths(manifest_root: Path, experiment: str, fold: int) -> list[Path]:
    ...
    return [folder / f"client_{site}_train_manifest.csv" for site in SITES] + [
        folder / "global_test_manifest.csv"
    ]


def check_inputs(setup: dict, manifest_root: Path, prefix_map: dict[str, str], *, check_features=True) -> dict:
    """Validate private manifests without loading feature arrays."""
    ...
    return {"patients": len(test_patients), "fold_counts": counts,
            "feature_files": len(feature_files), "feature_bytes": total_bytes}
...
def training_args(root: Path, setup: dict, output_dir: Path, manifest_root: Path | None = None) -> SimpleNamespace:
    ...
    if setup.get("variant") == "topk":
        ...
        settings.update(model_variant="topk", dtfd_top_k=options["top_k"],
    ...
    settings.update(output_root=str(root), manifest_dir=str(manifest_root / setup["experiment"]),
                    out_dir=str(inside(inside(root, "results"), output_dir)), force=False)
    return SimpleNamespace(**settings)


def make_spec(core: SimpleNamespace, root: Path, setup: dict, output_dir: Path,
    ...
    return core.experiment_utils.ModelSpec(
        ...
        mil_label="Top-k two-tier MIL" if is_topk else original.mil_label, model_name=setup["model_name"],
        source_run_dir=manifest_root / setup["experiment"], out_dir=output_dir,
    )
