...
from fabric_common import (
    SITES, cpu_state, inside, load_core, load_setup, local_seed, read_json, state_digest, write_json,
)
...
class FabricFedAvg(ModelAggregator):
    def __init__(self, runtime_path: str):
        ...
        self.setup = load_setup(self.root, self.runtime["experiment"],
                                self.runtime.get("variant", "pooling"), self.runtime.get("top_k"))
        self.core = load_core()
    ...
    def accept_model(self, model: FLModel):
        ...
        if model.meta.get("seed") != local_seed(self.setup, self.runtime["fold"], self.next_round + self.round_offset, site):
    ...
    def aggregate_model(self) -> FLModel:
        ...
        checkpoint = {"model_state_dict": state, "config": self.runtime["model_config"],
                      "fold": self.runtime["fold"], "condition": self.setup["model_key"],
                      "completed_rounds": original_round, "state_sha256": digest}
        ...
        write_json(self.fold_dir / "server_audit" / f"round_{original_round}.json", {
            "fold": self.runtime["fold"], "round": original_round,
            "sites_in_aggregation_order": list(SITES), "patient_weights": weights,
            "input_state_sha256": self.expected_digest, "output_state_sha256": digest,
            "local_logs": logs,
        })
        ...
        print(f"[FABRIC FedAvg] fold={self.runtime['fold']} round={original_round} "
              f"sites={list(SITES)} patient_weights={weights}", flush=True)
        self.updates = {}
        result = FLModel(params=state, params_type=ParamsType.FULL,
