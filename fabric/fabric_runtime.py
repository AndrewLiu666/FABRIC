"""Short local IPC paths and a checked launcher for the same FLARE simulator."""
...
def create_runtime_tmp(root: Path, run_root: Path) -> Path:
    ...
    write_json(run_root / "runtime_paths.json", {
...
def launch_simulator(job_dir: Path, workspace: Path, log_path: Path) -> Path:
    ...
    """
    command = [sys.executable, "-B", "-u", "-m", "nvflare.private.fed.app.simulator.simulator",
               str(job_dir), "-w", str(workspace), "-c", ",".join(SITES),
               "-n", str(len(SITES)), "-t", "1"]
    log_path.parent.mkdir(parents=True, exist_ok=True)
    recent_lines = deque(maxlen=25)
    record = {"command": command, "workspace": str(workspace), "log": str(log_path),
              "started_utc": datetime.now(timezone.utc).isoformat(), "returncode": None}
    process = None
    try:
        with log_path.open("x", encoding="utf-8", buffering=1) as handle:
            header = f"[FABRIC simulator] {shlex.join(command)}\n"
            handle.write(header)
            ...
            try:
                ...
                # the launcher fails while the simulator is still alive.
        ...
        if record["returncode"] != 0:
            raise RuntimeError(
                f"FLARE simulator exited with code {record['returncode']}; "
                f"no results will be evaluated. Full startup/training log: {log_path}\n"
                + "\n".join(recent_lines)
            )
        if not workspace.is_dir():
            raise RuntimeError(f"FLARE simulator produced no workspace. Inspect {log_path}")
        return workspace
