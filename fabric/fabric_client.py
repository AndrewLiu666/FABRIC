...
from fabric_common import (
    SITES, check_environment, cpu_state, inside, install_feature_reader, load_core, load_setup,
    local_seed, make_spec, read_json, sha256_file, state_digest, training_args, write_json,
)
...
def main() -> None:
    ...
    try:
        ...
        label = (f"{setup['encoder']} fold_index={runtime['fold']} "
                 f"round={round_idx + 1}/{train_args.rounds} site={site} local_train")
        with loader_progress(core.client_training, epochs=train_args.local_epochs, label=label,
                             log_path=log_dir / f"round_{round_idx + 1}_progress.log"):
            state, log = core.client_training.train_local_client_final(
                core.experiment_utils.condition_for(spec), global_state, client_df,
                setup["model_name"], setup["input_dim"], train_args, device, seed=seed,
            )
        state = cpu_state(state)
        log.update(fold=runtime["fold"], round=round_idx + 1, site=site,
                   model=setup["model_key"], algorithm="fedavg", seed=seed,
                   local_epochs=train_args.local_epochs,
                   input_state_sha256=input_digest, output_state_sha256=state_digest(state))
        write_json(log_path, log)
        print(f"[FABRIC] fold={runtime['fold']} round={round_idx + 1} site={site} "
              f"patients={len(client_df)} loss={log['loss']:.6f}", flush=True)
        flare.send(FLModel(
            params=state, params_type=ParamsType.FULL, current_round=flare_round,
            metrics={"train_loss": float(log["loss"])},
            meta={"site": site, "train_patients": len(client_df), "seed": seed,
                  "local_epochs": train_args.local_epochs, "input_state_sha256": input_digest,
                  "local_log": log},
        ))
    finally:
        flare.shutdown()
