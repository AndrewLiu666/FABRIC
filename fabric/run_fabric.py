...
from fabric_common import (
    EXPERIMENTS, SITES, VARIANTS, check_environment, check_inputs, cpu_state, inside, install_feature_reader,
    load_data_config, load_core, load_setup, make_spec, read_json, remap_feature_paths,
    sha256_file, state_digest, training_args, write_json,
)
...
def build_recipe(runtime_path: Path, setup: dict):
    ...
    model_args = {
        "model_name": setup["model_name"], "input_dim": setup["input_dim"],
        "n_classes": 2, "embed_dim": settings["embed_dim"],
        "attn_dim": settings["attn_dim"], "dropout": settings["dropout"],
        "dtfd_pseudo_bags": settings["dtfd_pseudo_bags"],
    }
    if setup.get("variant") == "topk":
        model_args.update(dtfd_top_k=setup["model_options"]["top_k"],
                          dtfd_eval_group_seed=setup["model_options"]["eval_group_seed"])
    recipe = FedAvgRecipe(
        ...
        min_clients=len(SITES), num_rounds=settings["rounds"] - runtime.get("resume_offset", 0),
        ...
        launch_external_process=True, launch_once=False,
        command=shlex.join([sys.executable, "-B", "-u"]),
        server_expected_format=ExchangeFormat.PYTORCH,
        params_transfer_type=TransferType.FULL,
        per_site_config={site: {} for site in SITES},
        key_metric="", stop_cond=None, patience=None,  # Disable best-model selection as well.
    )
...
def run_fold(root: Path, setup: dict, fold: int, output_dir: Path, core,
             manifest_root: Path, prefix_map: dict[str, str], *, resume: bool = False) -> None:
    import pandas as pd
    import torch
    ...
    client_dfs = {
        site: pd.read_csv(fold_dir / f"client_{site}_train_manifest.csv") for site in SITES
    }
    ...
    if recovery["action"] == "start":
        ...
        model_config = {
            "model": setup["model_name"], "encoder": setup["encoder"],
            "input_dim": setup["input_dim"],
            "protocol": "dtfd_topk_epoch5_fedavg_nvflare" if setup.get("variant") == "topk" else "dtfd_epoch5_fedavg_nvflare",
            "settings": vars(train_args),
        }
        runtime = {
            "root": str(root), "experiment": setup["experiment"], "fold": fold,
            "variant": setup.get("variant", "pooling"), "top_k": setup.get("model_options", {}).get("top_k"),
            "fold_dir": str(fold_dir), "model_config": model_config,
            "manifest_root": str(manifest_root),
            "client_patients": {site: len(df) for site, df in client_dfs.items()},
            "initial_state_sha256": state_digest(initial_state),
            "state_spec": {key: {"shape": list(value.shape), "dtype": str(value.dtype)}
        ...
        print(f"[resume] {setup['encoder']} fold={fold} completed_rounds={recovery['completed_rounds']} "
              f"action={recovery['action']}", flush=True)
    ...
    if recovery["completed_rounds"] < train_args.rounds:
        ...
        try:
            ...
            env = SimEnv(clients=list(SITES), num_threads=1,
                         workspace_root=str(work_dir / "nvflare_workspace"))
            recipe.export(str(work_dir / "exported_job"), env=env)
            workspace = inside(fold_dir, Path(env.workspace_root) / recipe.job.name)
            launch_simulator(work_dir / "exported_job" / recipe.job.name, workspace,
        ...
        if not (fold_dir / "global_model_round_final.pt").is_file():
            ...
            runtime = read_json(runtime_path)
            atomic_save({"model_state_dict": state, "config": runtime["model_config"],
                         "fold": fold, "condition": setup["model_key"], "completed_rounds": train_args.rounds,
                         "state_sha256": recovery["state_sha256"]}, fold_dir / "global_model_round_final.pt")
    ...
    framework_files = ([Path(recovery["framework_checkpoint"])] if recovery["action"] == "evaluate"
    ...
    with loader_progress(core.training, epochs=1,
        ...
        metrics, predictions = core.training.evaluate_model(
            model, test_df, pd.concat(client_dfs.values(), ignore_index=True), train_args,
            seed=train_args.seed + fold, device=device,
        )
    ...
    row = core.client_training.result_row(
        core.experiment_utils.condition_for(spec), fold, output_dir, metrics, predictions, seconds,
    )
    ...
    core.experiment_utils.write_fold_runtime(fold_dir, row, start_stamp,
                                    datetime.now(timezone.utc).isoformat(), seconds)
    write_json(fold_dir / "nvflare_run.json", {
        "completion_evidence": "5 server audits + 15 client updates + matching FLARE final checkpoint",
        "workspace": str(workspace), "framework_final_checkpoint": str(framework_files[0]),
        ...
        **({"resumed_from_round": recovery["completed_rounds"],
           ...
           if recovery["action"] != "start" else {}),
    })
    core.experiment_utils.refresh_summaries(spec, train_args.threshold)
    print(f"[completed] {setup['encoder']} fold={fold} auc={metrics['auc']:.6f} "
          f"bacc={metrics['bacc']:.6f} acc={metrics['acc']:.6f}", flush=True)


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--experiment", choices=[*EXPERIMENTS, "both"], default="both")
    parser.add_argument("--variant", choices=VARIANTS, default="pooling",
                        help="pooling: attention-weighted aggregation; topk: supervised high/low patch selection")
    parser.add_argument("--top-k", type=int,
                        help="Top-k variant only: patches per high/low set per pseudo-bag (default: 1)")
    ...
    parser.add_argument("--data-config", type=Path, default=Path("configs/data_paths.local.json"),
    ...
    if args.resume:
        ...
        for setup in setups:
            for fold in args.folds:
                ...
                print(f"[resume plan] {setup['encoder']} fold={fold}: {plan['action']}; "
                      f"completed_rounds={plan['completed_rounds']}/5", flush=True)
    print(json.dumps({"mode": "execute" if args.execute else "plan_only",
                      "experiments": list(experiments), "folds": args.folds,
                      "variant": args.variant, "model_options": setups[0].get("model_options", {}),
                      "rounds": 5, "local_epochs": 5, "sites": list(SITES),
                      "inputs": checks, "manifests": str(manifest_root),
                      "features": "external, read-only"}, indent=2))
    if not args.execute:
        print("No model imported, no training or simulator started. Use --execute only when ready.")
        return
...
def execute_run(args, run_root: Path, setups: list, manifest_root: Path, prefix_map: dict) -> None:
    ...
    write_json(metadata_root / "code_sha256.json", {
    ...
    for setup in setups:
        ...
        if args.resume:
            remaining = [fold for fold in args.folds
                         if inspect_fold(output_dir / f"fold_{fold}", setup)["action"] != "complete"]
            if not remaining:
                ...
                newest_fold = max((output_dir / f"fold_{fold}" / "nvflare_run.json").stat().st_mtime
                                  for fold in args.folds)
                if any(not (output_dir / n).is_file() or (output_dir / n).stat().st_mtime < newest_fold
                       for n in summaries):
                    core.experiment_utils.refresh_summaries(make_spec(core, ROOT, setup, output_dir, manifest_root),
                                                   setup["settings"]["threshold"])
                continue
        else:
            remaining = args.folds
        output_dir.mkdir(exist_ok=args.resume)
        config = {
            "schema_version": 1, "experiment": setup["experiment"],
            "model_name": setup["model_name"], "encoder": setup["encoder"],
            "variant": setup.get("variant", "pooling"), "model_options": setup.get("model_options", {}),
            "input_dim": setup["input_dim"],
            "settings": vars(training_args(ROOT, setup, output_dir, manifest_root)),
            "folds": args.folds, "source_fold_manifests": str(manifest_root),
            "execution_framework": "NVIDIA FLARE 2.7.2",
        }
        if not (args.resume and (output_dir / "run_config.json").is_file()):
            write_json(output_dir / "run_config.json", config)
        progress = Progress(len(remaining), f"{setup['encoder']} selected_folds",
                            output_dir / "logs/progress.log", unit="fold")
    ...
    write_json(run_root / "completed.json", {"experiments": [s["experiment"] for s in setups], "folds": args.folds,
