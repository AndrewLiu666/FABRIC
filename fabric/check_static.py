...
CRITICAL_DEFINITIONS = {
    ...
    "training.py": {
        "PatientFeatureDataset", "build_model", "build_criterion", "compute_metrics",
        "evaluate_model", "forward_bags", "make_loader", "read_patient_bag",
    },
    "client_training.py": {"train_local_client_final"},
    "experiment_utils.py": {
        "condition_for", "install_hdf5_safe_reader", "normalized_result_row",
        "refresh_summaries", "safe_read_feature_array",
    },
}
...
def _check_nvflare_api() -> None:
    ...
    checks = (
        (
            ...
            "FedAvgRecipe",
            {
                "model", "initial_ckpt", "min_clients", "num_rounds", "train_script",
                "train_args", "aggregator", "launch_external_process", "launch_once",
                "command", "server_expected_format", "params_transfer_type",
                "per_site_config", "key_metric", "stop_cond", "patience",
            },
        ),
        (
